//
//  DataChannelMessageVC.swift
//  ARTCExample
//
//  Created by wy on 2025/7/15.
//

import Foundation
import AliVCSDK_ARTC

class DataChannelMessageSetParamsVC: UIViewController, UITextFieldDelegate {

    override func viewDidLoad() {
        super.viewDidLoad()

        // Do any additional setup after loading the view.
        self.title = "Common Message Sending And Receiving".localized
    ...
    @IBAction func onJoinChannelBtnClicked(_ sender: UIButton) {
        ...
        let vc = self.presentVC(storyboardName: "DataChannelMessage", storyboardId: "MainVC") as? DataChannelMessageMainVC
...
class DataChannelMessageMainVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.commonMsgInputTextField.delegate = self
        
        self.setup()
        self.enableDataChannel()
        self.startPreview()
        self.joinChannel()
    }
    ...
    @IBOutlet weak var commonMsgInputTextField: UITextField!
    @IBAction func onCommonMsgSendButtonClicked(_ sender: UIButton) {
        guard let dataChannelMsg = self.commonMsgInputTextField.text, dataChannelMsg.isEmpty == false else {
            return
        }
        
        if self.sendDataChannelMessage(msg: dataChannelMsg) {
            UIAlertController.showAlertWithMainThread(msg: "Send Succeed", vc: self)
        }
        else {
            UIAlertController.showAlertWithMainThread(msg: "Failed to Send", vc: self)
        }
    }
    ...
    // 开启dataChannel通道
    func enableDataChannel() {
        let enablePub = true
        let enableSub = true
        // Json字符串
        let parameter = "{\"data\":{\"enablePubDataChannel\":true,\"enableSubDataChannel\":true}}"
        self.rtcEngine?.setParameter(parameter)
    }
    
    // 发送DataChannel消息，以文本消息为例
    func sendDataChannelMessage(msg: String) -> Bool {
        let dataChannelMsg = AliRtcDataChannelMsg()
        dataChannelMsg.type = .custom
        if let data = msg.data(using: .utf8) {
            dataChannelMsg.data = data
        }
        let ret = self.rtcEngine?.sendDataChannelMessage(dataChannelMsg)
        return ret == 0
    }
...
extension DataChannelMessageMainVC: UITextFieldDelegate {
...
extension DataChannelMessageMainVC: AliRtcEngineDelegate {
    
    // 远端用户订阅datachannel回调，表示可以开始发送datachannel消息
    // receive DataChannel message
    func onRemoteUserSubscribedDataChannel(_ uid: String) {
        "onRemoteUserSubscribeDataChannel uid: \(uid)".printLog()
    }
    
    // 收到DataChannel消息回调
    // receive DataChannel message
    func onDataChannelMessage(_ uid: String, controlMsg: AliRtcDataChannelMsg) {
        // TODO 编写自定义消息处理
        if controlMsg.type == .custom {
            guard let message = String(data: controlMsg.data, encoding: .utf8) else {
                print("无法解析消息")
                return
            }
            self.showToast(message: "Received Data Channel Message: \(message), from uid: \(uid)")
        }
    }
