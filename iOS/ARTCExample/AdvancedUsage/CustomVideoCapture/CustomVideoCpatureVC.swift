...
extension CustomVideoCaptureMainVC: CameraCaptureHelperDelegate {
    ...
    func videoCapture(_ capture: CameraCaptureHelper, didCaptureOutputBuffer sampleBuffer: CMSampleBuffer) {
        ...
        guard let (nv12Ptr, width, height, totalSize) = copyNV12Data(from: pixelBuffer) else{return}
        
        let dataSample = AliRtcVideoDataSample()
        dataSample.dataPtr = nv12Ptr
        dataSample.format = .NV12
        dataSample.type = .raw_Data
        dataSample.width = Int32(width)
        dataSample.height = Int32(height)
        dataSample.dataLength = totalSize
        dataSample.rotation = 90
        // 向SDK输入外部视频源
