...
public class DataChannelMessageActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mEditText= findViewById(R.id.message_input0);
        mSendButton= findViewById(R.id.send_btn0);
...
//            mAliRtcEngine.sendMediaExtensionMsgEx(seiData,2,0,false, 5);
            mEditText.setText("");
        });
    }
    ...
    private void startCustomMsgChannel() {
        String param = "{\"data\":{\"enablePubDataChannel\":true" + ",\"enableSubDataChannel\":true}}";
        if (mAliRtcEngine != null && mAliRtcEngine.setParameter(param) == 0){
//            ToastHelper.showToast(this, getString(R.string.custom_msg_channel_success), Toast.LENGTH_SHORT);
        }
    }
...
//在AliRtcEngineNotify回调中
        public void onDataChannelMessage(String uid, AliRtcEngine.AliRtcDataChannelMsg msg) {
            ...
            handler.post(new Runnable() {
                @Override
                public void run() {
                    String receivedMsg = new String(msg.data); // 指定编码格式
                    ToastHelper.showToast(DataChannelMessageActivity.this, receivedMsg, Toast.LENGTH_SHORT);
                }
            });
        }
    };
